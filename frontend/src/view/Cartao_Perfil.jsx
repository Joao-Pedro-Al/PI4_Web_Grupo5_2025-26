// Cartao_Perfil.jsx - VERSÃO ULTRA SIMPLES
import { Link } from 'react-router-dom';

const CartaoPerfil = ({ nome, num, email, idclasse, idperf }) => {
    if (num == null) { num = "N/A" }
    if (email == null) { email = "N/A" }

    // URL ABSOLUTAMENTE FIXA - SEM LOGICA COMPLEXA
    const perfilUrl = `/backoffice/perfis/${idperf}`;

    const classeCSS = idclasse == 1 ? "paciente" : "doutor";

    return (
        <div className={`card div--cartao--perfil d-block col-sm-11 col-lg-5 col-xl-4 ${classeCSS}`}>
            <Link to={perfilUrl} className="text-decoration-none div__a--link">
                <div className="card-body py-4">
                    <h5 className="card-title fw-bold mb-3">{nome}</h5>
                    <p className="card-text mb-2"><b>Telefone: </b>{num}</p>
                    <p className="card-text"><b>Email: </b>{email}</p>
                </div>
            </Link>
        </div>
    );
}

export default CartaoPerfil;